...
class ModelsRegistry {
  ...
  async listModels() {
    ...
    if (source !== "static") {
      try {
        gatewayModels = await this._fetchGatewayModels();
      } catch (error) {
    ...
    if (source === "static") {
      this._cache = mergeModels(staticModels);
    } else if (source === "hybrid") {
      this._cache = mergeModels(gatewayModels, staticModels);
    } else {
      this._cache = mergeModels(gatewayModels, gatewayModels.length > 0 ? [] : staticModels);
    }
